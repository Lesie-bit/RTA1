...
export default function GuideCategoriesManager({
  categories,
  setCategories,
}: {
  categories: GuideCategory[];
  setCategories: React.Dispatch<React.SetStateAction<GuideCategory[]>>;
}) {
  ...
  function startEdit(c: GuideCategory) {
    setEditingId(c.id);
  ...
  async function handleDelete(id: string) {
    if (
      !confirm(
        "ลบหมวดหมู่ย่อยนี้? เนื้อหาวิธีเล่นในหมวดนี้จะไม่ถูกลบ แต่จะกลายเป็น 'ไม่มีหมวดหมู่'"
      )
    )
      return;
    ...
    <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_360px]">
      <div className="space-y-2">
        {categories.length === 0 && (
          <p className="text-sm text-[var(--color-slate-500)]">
            ยังไม่มีหมวดหมู่ย่อย เริ่มเพิ่มได้จากฟอร์มด้านขวา
          </p>
        )}
        {categories.map((c, i) => (
          <div
      ...
      <div className="h-fit rounded border border-[var(--color-ink-700)] bg-[var(--color-ink-900)] p-5">
        <p className="font-display text-sm text-[var(--color-paper-50)]">
          {editingId ? "แก้ไขหมวดหมู่ย่อย" : "เพิ่มหมวดหมู่ย่อยใหม่"}
        </p>

        <div className="mt-4 space-y-3">
          <input
            placeholder="ชื่อหมวดหมู่ เช่น เริ่มต้นเล่น, อาชีพ"
            value={form.name}
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              ...
              {saving ? "กำลังบันทึก..." : editingId ? "บันทึกการแก้ไข" : "เพิ่มหมวดหมู่ย่อย"}
            </button>
            {editingId && (
              <button
                onClick={resetForm}
